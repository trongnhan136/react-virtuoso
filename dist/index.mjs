...
function useSizeWithElRef(callback, enabled = true, externalWindow) {
...
function useSize(callback, enabled = true, externalWindow) {
...
function useChangedListContentsSizes(callback, itemSize, enabled, scrollContainerStateCallback, log, gap, customScrollParent, externalWindow) {
  const memoedCallback = React.useCallback(
    (el) => {
      ...
      const w = externalWindow != null ? externalWindow : window;
      const windowScrolling = scrollableElement.lastElementChild.dataset["viewportType"] === "window";
      const scrollTop = customScrollParent ? customScrollParent.scrollTop : windowScrolling ? w.pageYOffset || w.document.documentElement.scrollTop : scrollableElement.scrollTop;
      const scrollHeight = customScrollParent ? customScrollParent.scrollHeight : windowScrolling ? w.document.documentElement.scrollHeight : scrollableElement.scrollHeight;
      const viewportHeight = customScrollParent ? customScrollParent.offsetHeight : windowScrolling ? w.innerHeight : scrollableElement.offsetHeight;
...
function useWindowViewportRectRef(callback, customScrollParent, externalWindow) {
...
const GridItems = /* @__PURE__ */ React.memo(function GridItems2() {
  ...
  const externalWindow = useEmitterValue$1("externalWindow");
...
const TableRoot = /* @__PURE__ */ React.memo(function TableVirtuosoRoot(props) {
  ...
  const externalWindow = useEmitterValue("externalWindow");
  const context = useEmitterValue("context");
  const theadRef = useSize(compose(fixedHeaderHeight, (el) => correctItemSize(el, "height")), true, externalWindow);
  const tfootRef = useSize(compose(fixedFooterHeight, (el) => correctItemSize(el, "height")), true, externalWindow);
